...
describe('AbstractFormBaseComponent', () => {
  ...
  it('should update and clear formErrors with updateFormErrors', () => {
    if (!component) {
      fail('component returned null');
      return;
    }

    const errs = [{ fieldId: 'court', message: 'Court is required', priority: 1, type: 'required' as const }];

    component.updateFormErrors(errs);
    expect(component.formErrors).toEqual(errs);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    component.updateFormErrors(undefined as unknown as any);
    expect(component.formErrors).toEqual([]);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    component.updateFormErrors(null as unknown as any);
    expect(component.formErrors).toEqual([]);
  });

  it('mergeFieldErrors should merge by key without overwriting existing error types', () => {
    if (!component) {
      fail('component returned null');
      return;
    }
    ...
    const current = {
      court: { required: { message: 'Select a court', priority: 1 } },
      surname: { required: { message: 'Enter surname', priority: 1 } },
    };

    const incoming = {
      court: { minlength: { message: 'Too short', priority: 2 } },
      forename: { required: { message: 'Enter forename', priority: 1 } },
    };

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const merged = component['mergeFieldErrors'](current as any, incoming as any);

    expect(merged['court']['required'].message).toBe('Select a court');
    expect(merged['court']['minlength'].message).toBe('Too short');
    expect(merged['surname']['required'].message).toBe('Enter surname');
    expect(merged['forename']['required'].message).toBe('Enter forename');

    // New object reference is returned
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect(merged).not.toBe(current as any);
  });
  it('should set fieldErrors to empty object when updateFieldErrors is called with falsy value', () => {
    if (!component) {
      fail('component returned null');
      return;
    }

    // Prime with some data so we can verify replacement with empty object
    component.updateFieldErrors({
      court: { required: { message: 'x', priority: 1 } },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as unknown as any);

    // Now call with undefined to hit the fallback branch
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    component.updateFieldErrors(undefined as unknown as any);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect((component as any)['fieldErrors']).toEqual({});
  });

  it('should set formControlErrorMessages to empty object when updateFormControlErrorMessages is called with falsy value', () => {
    if (!component) {
      fail('component returned null');
      return;
    }

    component.updateFormControlErrorMessages({ court: 'Court is required' });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    component.updateFormControlErrorMessages(undefined as unknown as any);
    expect(component.formControlErrorMessages).toEqual({});
  });

  it('should set formErrorSummaryMessage to empty array when updateFormErrorSummaryMessage is called with non-array or falsy', () => {
    if (!component) {
      fail('component returned null');
      return;
    }

    component.updateFormErrorSummaryMessage([{ fieldId: 'court', message: 'Court is required' }]);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    component.updateFormErrorSummaryMessage(undefined as unknown as any);
    expect(component.formErrorSummaryMessage).toEqual([]);
  });

  it('mergeFieldErrors should handle undefined current and incoming, and ignore undefined defs', () => {
    if (!component) {
      fail('component returned null');
      return;
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const merged1 = (component as any)['mergeFieldErrors'](undefined as any, undefined as any);
    expect(merged1).toEqual({});

    const current = {
      court: { required: { message: 'Select a court', priority: 1 } },
    };
    // Include an entry with `undefined` defs to exercise `(defs ?? {})`
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const incoming: any = { surname: { required: { message: 'Enter surname', priority: 1 } }, forename: undefined };

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const merged2 = (component as any)['mergeFieldErrors'](current as any, incoming as any);

    expect(merged2['court']['required'].message).toBe('Select a court');
    expect(merged2['surname']['required'].message).toBe('Enter surname');
    expect(merged2['forename']).toEqual({});
  });
});
