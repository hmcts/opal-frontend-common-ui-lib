...
This Angular component provides a live region for announcing dynamic content to assistive technologies. It supports status and alert roles and defers population of the live region to allow assistive technologies to register the region before its content changes
...
```html
<opal-lib-custom-deferred-live-region-announcement [message]="NO_RESULTS_ANNOUNCEMENT" role="status" ></opal-lib-custom-deferred-live-region-announcement>
```
...
The live region is initially rendered without announcement text. When `message` is provided or changes, the existing content is cleared and the new message is populated after a short delay, allowing assistive technologies to detect the content change.
...
The live region also uses `aria-atomic="true"` so that the complete announcement is presented when its content changes.

Pending announcements are cleared when the component is destroyed.
